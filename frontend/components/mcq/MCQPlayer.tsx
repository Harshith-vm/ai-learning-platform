...
export function MCQPlayer({ documentId }: MCQPlayerProps) {
  ...
  const handleSubmit = useCallback(() => {
    ...
    const correctIndex = currentMCQ.options.findIndex(opt => opt.is_correct);
    setCorrectOptionIndex(correctIndex);

    // Check if selected answer is correct
    const isCorrect = currentMCQ.options[selectedOption].is_correct;

    // Add slight delay for feedback animation
    setTimeout(() => {
      setIsAnswered(true);
  ...
  }, [selectedOption, isAnswered, mcqs, currentIndex, score]);
